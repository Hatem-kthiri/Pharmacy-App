...
// Get all products provided by the current provider
router.get("/get-products", authenticateUser, async (req, res) => {
  try {
    const products = await Product.find({ provider: req.user.id }).populate(
      "provider",
      "name"
    );
    res.json(products);
...
router.post(
  ...
  async (req, res) => {
    try {
      const { name, description, quantity, price, manufacturer, expiryDate } =
        req.body;
      ...
      const product = new Product({
        ...
        provider: req.user.id, // Associate the product with the current provider
      });
